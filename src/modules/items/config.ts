...
/** Vente rapide au serveur : l'objet est détruit contre une part de son prix. */
const sellSchema = z.object({
  /** Autoriser `/vendre` (vente rapide au serveur). */
  enabled: z.boolean().default(true),
  /** Part du prix boutique reversée à la vente (0-100 %). */
  percent: z.number().int().min(0).max(100).default(40),
});

export type SellConfig = z.infer<typeof sellSchema>;
