...
/** Autocomplétion depuis l'INVENTAIRE du membre (objets possédés ayant un prix). */
...
export const vendre: SlashCommand = {
  ...
  async execute(interaction, ctx) {
    if (!interaction.inGuild()) return;
    const itemsConfig = await getItemsConfig(ctx, interaction.guildId);
    if (!itemsConfig.sell.enabled) {
      await interaction.reply({
        content: t('modules.items.commands.sell.disabled'),
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    const econ = await getEconomyConfig(ctx, interaction.guildId);
    const result = await sellToServer(
      ctx,
      interaction.guildId,
      interaction.user.id,
      item,
      qty,
      itemsConfig.sell.percent,
    );

    if (!result.ok) {
      const key =
        result.reason === 'noValue'
          ? 'modules.items.commands.sell.noValue'
          : 'modules.items.commands.sell.notOwned';
      await interaction.reply({
        content: t(key, { emoji: item.emoji, name: item.name }),
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    await interaction.reply({
      content: t('modules.items.commands.sell.done', {
        qty: result.sold,
        emoji: item.emoji,
        name: item.name,
        percent: itemsConfig.sell.percent,
        total: formatMoney(econ, result.total),
        balance: formatMoney(econ, result.balance ?? 0),
      }),
    });
  },
};
