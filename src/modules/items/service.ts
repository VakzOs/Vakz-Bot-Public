...
// --- Vente rapide au serveur ------------------------------------------------

/** Valeur de revente d'UN exemplaire : part `percent` (0-100) du prix, arrondie bas. */
export function sellUnitValue(price: number, percent: number): number {
  return Math.max(0, Math.floor((price * percent) / 100));
}

export interface SellResult {
  ok: boolean;
  reason?: 'noValue' | 'notOwned';
  sold: number;
  unitValue: number;
  total: number;
  balance: number | null;
...
/**
 * Vente rapide au serveur : détruit jusqu'à `qty` exemplaires de `item`
 * réellement possédés et crédite `unitValue × vendus` sur le solde. Le retrait
 * de l'inventaire est atomique (`takeFromInventory`).
 */
export async function sellToServer(
  ctx: BotContext,
  guildId: string,
  userId: string,
  item: Item,
  ...
  const unitValue = sellUnitValue(item.price, percent);
  if (unitValue <= 0) {
    return { ok: false, reason: 'noValue', sold: 0, unitValue, total: 0, balance: null };
  }
  const owned = await getQuantity(ctx, guildId, userId, item.id);
  const sold = Math.min(Math.max(1, qty), owned);
  if (sold <= 0 || !(await takeFromInventory(ctx, guildId, userId, item.id, sold))) {
    return { ok: false, reason: 'notOwned', sold: 0, unitValue, total: 0, balance: null };
  }
  const total = unitValue * sold;
  const balance = await addBalance(ctx, guildId, userId, total);
  return { ok: true, sold, unitValue, total, balance };
}
