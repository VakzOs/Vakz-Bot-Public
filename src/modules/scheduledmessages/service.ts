...
/**
 * Après une écriture depuis le dashboard : amorce `lastPosted` des messages qui
 * viennent d'être créés. Sans cela, un message « toutes les N heures » créé avec
 * `lastPosted = 0` serait considéré comme dû et partirait au tick suivant.
 * Renvoie la liste corrigée, ou `null` si rien ne change.
 */
export function primeNewMessages(
  config: ScheduledmessagesConfig,
  previous: ScheduledmessagesConfig,
): ScheduledMessage[] | null {
  const known = new Set(previous.messages.map((message) => message.id));
  let changed = false;
  const messages = config.messages.map((message) => {
    if (known.has(message.id) || message.lastPosted !== 0) return message;
    const lastPosted = initialLastPosted(message.schedule);
    if (lastPosted === 0) return message;
    changed = true;
    return { ...message, lastPosted };
  });
