...
import { type DucklingReport, applyDucklingToMove } from './duckling.js';
...
export interface MoveOutcome {
  ...
  /**
   * Vrai quand l'événement PROMETTAIT un cadeau (`item`) mais que rien n'est
   * tombé : barème de drop trop bas, aucun objet « Drop en jeu » au catalogue,
   * ou `giveItems` désactivé. Le texte de l'événement annonce un présent, donc
   * l'absence doit être dite — sinon le joueur croit à un bug.
   */
  missedItem: boolean;
  fainted: boolean;
  /** Vrai si le voyageur est parti avec une énergie NÉGATIVE (malus appliqués). */
  exhausted: boolean;
  /** Détail du canneton collé au voyageur, s'il y en a un. */
  duckling: DucklingReport | null;
  traveler: Traveler;
}
...
export async function move(ctx: BotContext, guildId: string, userId: string): Promise<MoveOutcome> {
  ...
  // Canneton collé : rogne les gains (distance, pièces) et amplifie les dégâts,
  // puis consomme un de ses tours. Modifie `deltas` en place.
  const duckling = await applyDucklingToMove(ctx, guildId, userId, deltas);
  ...
  // Le butin du canneton revient à celui qui l'a collé (jamais à sa victime).
  if (config.giveCoins && duckling && duckling.stolenCoins > 0 && duckling.ownerId) {
    await addBalance(ctx, guildId, duckling.ownerId, duckling.stolenCoins);
  }
  ...
  return {
    ...
    deltas,
    itemFound,
    missedItem: Boolean(event.item) && itemFound === null,
    fainted,
    exhausted,
    duckling,
    traveler,
