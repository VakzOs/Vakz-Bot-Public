...
export const routeConfigSchema = z.object({
  ...
  /**
   * Ajouter une petite pique quand un événement PROMET un cadeau (marchand,
   * corbeau, fée…) mais que le tirage ne donne rien — barème trop bas, aucun
   * objet « Drop en jeu » au catalogue, ou objets désactivés.
   */
  missedGiftTaunt: z.boolean().default(true),
  /**
   * Piques personnalisées, UNE PAR LIGNE. Vide = celles fournies par le bot
   * (`modules.route.missedGift.lines`). Une ligne est tirée au hasard.
   */
  missedGiftMessages: z.string().max(4000).default(''),
});
