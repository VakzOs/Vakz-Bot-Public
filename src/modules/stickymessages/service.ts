...
function cancelRepost(channelId: string): void {
  const existing = pending.get(channelId);
  if (existing) {
...
/**
 * Après une écriture depuis le dashboard : pour chaque salon qui n'a plus de
 * sticky, annule le repost en attente et efface le dernier message posté —
 * sinon un sticky supprimé resterait affiché en bas du salon.
 */
export async function cleanupRemovedStickies(
  ctx: BotContext,
  guildId: string,
  config: StickymessagesConfig,
  previous: StickymessagesConfig,
): Promise<void> {
  const kept = new Set(config.stickies.map((sticky) => sticky.channelId));
  for (const sticky of previous.stickies) {
    if (kept.has(sticky.channelId)) continue;
    cancelRepost(sticky.channelId);
    if (!sticky.lastMessageId) continue;
    const channel = await ctx.client.channels.fetch(sticky.channelId).catch(() => null);
